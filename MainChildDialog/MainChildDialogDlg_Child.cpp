// MainChildDialogDlg_Child.cpp: 구현 파일
//
...
// MainChildDialogDlg_Child 메시지 처리기


BOOL MainChildDialogDlg_Child::OnInitDialog()
{
...
void MainChildDialogDlg_Child::OnPaint() {
	if (IsIconic())
	{
...
void MainChildDialogDlg_Child::SetupRC()
{
	int iSphere;
...
void MainChildDialogDlg_Child::DrawGround(void)
{
...
void MainChildDialogDlg_Child::RenderScene(void)
{
	//frameCamera.Normalize();
...
void MainChildDialogDlg_Child::objectAnimate(int i)
{
	if (i < 50)
	{
		for (int j = 0; j < 14; j++)
...
void MainChildDialogDlg_Child::myKeys(unsigned char key, int x, int y)
{
	switch (key)
	{
	case 'w':
...
void MainChildDialogDlg_Child::SpecialKeys(int key, int x, int y)
{
	if (key == GLUT_KEY_UP)
	{
...
void MainChildDialogDlg_Child::mouseClicked(int button, int state, int x, int y)
{
	if (state == GLUT_DOWN)
	{
...
int rotateCount = 0;
void MainChildDialogDlg_Child::mouseMotion(int x, int y)
{
	y = screenHeight - y;
...
void MainChildDialogDlg_Child::mouseScroll(int button, int dir, int x, int y)
{
	if (dir > 0)
...
void MainChildDialogDlg_Child::ChangeSize(int w, int h)
{
	GLfloat fAspect;
...
void MainChildDialogDlg_Child::OnDestroy() {
	CDialogEx::OnDestroy();
...
void MainChildDialogDlg_Child::OnTimer(UINT_PTR nIDEvent) {
...
int MainChildDialogDlg_Child::OnCreate(LPCREATESTRUCT lpCreateStruct) {
...
void MainChildDialogDlg_Child::OnSize(UINT nType, int cx, int cy) {
...
BOOL MainChildDialogDlg_Child::OnEraseBkgnd(CDC* pDC)
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
